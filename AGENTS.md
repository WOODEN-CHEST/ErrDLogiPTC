...
The project uses a custom library called WRFramework, it adds extra funcionality which raylib doesnt have, at least not fully.
The WRFramework headers are available under the include/wr directory.
The library is split into modules, each one having a header file and each for a specific task.
It should always be preferred to use this framework's funcionality where possible. If something should
exist in the framework but doesnt, let it be known so it can be added.
The framework is managed in a different repository, here only the headers exist.
...
Linux and Windows.
Platform dependent code should be in the WRFramework instead of the project itself. If platform
dependent code is required but does not exist in the framework, let it be know so it can be added.
...
- If you do not see a suitable error code for an error, it can be added to the WRFramework by bringing the issue up before proceeding.
